"""Live hook capture: an in-gateway spooler plus a Bridge-side drain.
...
- The **spooler** (``handler.py``, installed under ``$HERMES_HOME/hooks/``)
  ...
  Bridge home and never raises into the gateway. It imports nothing from
  this package, so it runs in whatever Python environment Hermes uses.
- The **drain** (:func:`drain`) runs in the Bridge environment inside
...
This keeps the encryption key and the sequence authority in Bridge, never
...
# All paths below are relative to the Bridge home (never HERMES_HOME).
